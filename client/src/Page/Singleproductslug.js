...
const Singleproductslug = () => {
  ...
  const singlep = async () => {
    try {
      const { data } = await axios.get(
        `/api/v1/getsingleproduct/${params.slug}`
      );
      if (data?.product) {
  ...
  const relatedpr=async()=>{
    try {
      if(product._id){
        const {data}=await axios.get(`/api/v1/getrelatedproducts/${product?._id}/${product?.category?._id}`);
        ...
        <div className="flex gap-[365px] flex-wrap w-[100vw] mt-[80px] ">
          <div className="border-[0px]  ml-[55px] mt-[10px] hah " onClick={() => {}}>
            {product?._id?(<img
              ...
              src={`/api/v1/getphoto/${product?._id}`}
              alt="photoff"
            ></img>):(<div> </div>)}
