...
const  Createcategory=()=> {
  ...
  async function hanler(e) {
    e.preventDefault();
    try{
        const re=await axios.post(`http://localhost:8000/api/v1/createcategory`,{name});
        if(re?.data?.success){
       getcategory();
        ...
        <div key={index}>
           ...
           <div>
             ...
             <button className="bg-red-600 absolute left-[520px] w-[60px] " onClick={async()=>{
                await axios.delete(`http://localhost:8000/api/v1/deletecategory/${c._id}`);
                getcategory();
             }}>Delete</button>
            </div>
            ...
            {!visible?(<div className=" h-[200px]  z-[1000] absolute top-[300px] left-[600px] translate-z-[100px]">
                <form onSubmit={async(e)=>{
                    ...
                    try{ const {data}=await axios.put(`http://localhost:8000/api/v1/updatecategory/${c1._id}`,{name:updatedname});
                    if(data?.success){
