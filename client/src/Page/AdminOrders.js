...
// const getpro=async()=>{
//   try {
//     const {data}=await axios.get(`/api/v1/getproductbyid`);
//     if(data.success){
//        console.log(data.d);
//     //    order=[data];
...
const buyerid=auth?.user?._id;
  const getOrders=async()=>{
    try {
        const {data}=await axios.get(`/api/v1/adminorder`,{buyerid});
        if(data.success){
           console.log(data.d);
        //    order=[data];
        data.d.map((c)=>{
  ...
  //     const {data}=await axios.post(`/api/v1/getproductbyid`,{p});
  ...
  //         products?.map(async(id) => axios.post(`/api/v1/getproductbyid`,{id}))
  //       );
...
//   const [stat,setstat]=useState(or?.status)
  const handlestatus=async(stat,id)=>{
    try {
        const {data}=await axios.put(`/api/v1/updateorder`,{id,stat})
        if(data.success){
            console.log(data);
        }
        else{
                    ...
                    st.map((c)=>(
                     ...
                     
                      {
                         pi?.products?.map((p,i)=>(
                          ...
                          <img className='w-[200px] h-[160px] rounded-[5%] border-b-[2px] mb-[15px] ml-[15px]' src={`/api/v1/getphoto/${p._id}`} alt='photoff'></img>
